    ...
    <div className="rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-card)] p-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-[color:var(--text-secondary)]">Últimos 6 meses · em mil R$</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barGap={6} margin={{ top: 10, right: 8, left: -16, bottom: 0 }}>
            ...
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              tickFormatter={(v: number) => (v === 0 ? "0k" : `${Math.round(v / 1000)}k`)}
            />
            <Tooltip
              ...
              formatter={(v: number, name) => [formatBRLCompact(v), name === "receita" ? "Receita" : "Despesa"]}
